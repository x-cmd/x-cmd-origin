...
function generate_advise_json_subcmd(indent, indent_str, indent_str2, indent_str4,
    ...
    for (i=1; i <= subcmd_len(); ++i) {
        ...
        _ret = _ret "\n" indent_str2 sprintf("%s: %s", qu( subcmd_id( i ) ),
            "{\n" indent_str4 str_wrap2("#desc") ": " subcmd_invocation      ) ","
    }
    return _ret
}
...
function generate_advise_json(      indent, indent_str,
    ...
    for (i=1; i <= namedopt_len(); ++i) {
        ...
        ADVISE_JSON = ADVISE_JSON "\n" indent_str2 str_wrap2( option_id ) ": {\n"
        for ( j=1; j<=option_argc; ++j ) {
            ADVISE_JSON = ADVISE_JSON indent_str4 sprintf("%s: %s", qu("#" j),
                generate_advise_json_value_candidates_by_rules(option_id SUBSEP j)    \
            ) ",\n"
        }

        ADVISE_JSON = ADVISE_JSON indent_str4 sprintf("%s: %s", str_wrap2("#desc"),
            qu(option_desc_get( option_id ))    \
        ) "\n" indent_str2 "},"
    }

    for (i=1; i <= flag_len() && option_id = flag_get( i ); ++i)
        ADVISE_JSON = ADVISE_JSON "\n" indent_str2 sprintf("%s: %s", str_wrap2(option_id),
            qu( "--- " option_desc_get( option_id ) ) \
        ) ","
    ...
    ADVISE_JSON = qu(ADVISE_JSON)
    gsub(/\002/, "\"", ADVISE_JSON)
    printf("printf %s %s; return 126;", str_wrap2("%s"), ADVISE_JSON)
}
## EndSection
