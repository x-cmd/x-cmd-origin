...
function generate_option_help(         _option_help, i, j, k, option_list, flag_list) {
    ...
    if (0 != flag_list[ LEN ]) {
        ...
        for (i=1; i<=flag_list[ LEN ]; ++i) {
            ...
            _option_help = _option_help "    \033[36m" _opt_help_doc_arr[ i ] _space "\t\033[91m" option_arr[option_id KSEP OPTION_DESC ] "\033[0m\n"
        }
    }
    ...
    if (0 != option_list[ LEN ]) {
        ...
        for (i=1; i<=option_list[ LEN ]; ++i) {
            ...
            _space     = get_space(_max_len-length(_opt_help_doc_arr[ i ]))
            _option_help = _option_help "    \033[36m" _opt_help_doc_arr[ i ] _space "\t\033[91m" option_arr[ option_list[ i ] KSEP OPTION_DESC ] "\033[0m" oparr_string _multiple"\n"
        }
    }
...
function generate_rest_argument_help(        _option_help) {
    ...
    for (i=1; i <= rest_option_id_list[ LEN ]; ++i) {
        ...
        oparr_string = oparr_string _default _candidate _regex
        _option_help = _option_help "    \033[36m" option_id _space "\t\033[91m" option_arr[option_id KSEP OPTION_DESC ] "\033[0m" oparr_string "\n"
    }
...
NR>=5 {
    ...
    if (keyline == "") {
        ...
        if (line_arr[1] == OBJECT_NAME) {
            keyline = option_alias_2_option_id[ "--" line_arr[2] ]
        }
    } else {
